// Integration tests for /embedding. Requires a running llama-server on the
// default local address, started with `--embedding`; see the server
// requirements in specs/endpoints/embedding.md. Both pooled and `--pooling none`
// launches are accepted, so `embedding` is asserted only as a non-empty array of
// float arrays.
